# ============================================================
# COMPLETE R SCRIPT aligned with gtm_bspline_year.cpp
# FIXES INCLUDED:
...
# ============================================================
# FIX FOR 2014 -> 2015 ONLY:
#   - optional smoothness penalty lambda_smooth
#   - optional support-based q_mask
#   - activated only for year y == 2014
# ============================================================
...
# Compile + load C++
# ----------------------------
dll_name <- dynlib("gtm_bspline_year")
try(dyn.unload(dll_name), silent = TRUE)
compile("gtm_bspline_year.cpp")
dyn.load(dll_name)
...
################################################################
# ============================================================
# Create appendix/supplementary LaTeX tables:
# rows = c1,...,c32,s_y,sigma
# columns = 4 years, each with subcolumns: theta-hat and SE
# Zero/fixed c_i shown as: estimate = 0, SE = -
# Structural zero detection uses NA in z or p_value
# s_y is marked with * because it is profiled, not estimated
# ============================================================

dir.create("tables_dir", showWarnings = FALSE)

# ------------------------------------------------------------
# 1) Parameter order and labels
# ------------------------------------------------------------
param_order <- c(
  "s_y",
  "sigma",
...
label_map <- setNames(param_labels, param_order)

# ------------------------------------------------------------
# 2) Compact formatter like 2.35e-2
# ------------------------------------------------------------
fmt_e <- function(x, digits = 2) {
  ...
  s <- sub("e\\+?", "e", s)   # e+02 -> e02
  s <- sub("e0+", "e", s)     # e02 -> e2
  s <- sub("e-0+", "e-", s)   # e-02 -> e-2
  s
}

# ------------------------------------------------------------
# 3) Prepare result table
#    Assumes all_results exists with:
#    year, name, estimate, se, z, p_value
# ------------------------------------------------------------
res_tab <- all_results[
  all_results$name %in% param_order,
  c("year", "name", "estimate", "se", "z", "p_value")
]

years_all <- sort(unique(res_tab$year))

# complete year/parameter combinations
...
# desired order
res_tab$name <- factor(res_tab$name, levels = param_order)
res_tab <- res_tab[order(res_tab$name, res_tab$year), ]
res_tab$name <- as.character(res_tab$name)

# ------------------------------------------------------------
# 4) Mark spline coefficients that were not estimated
#    Rule: q_full[i] with NA z or NA p_value
# ------------------------------------------------------------
is_q <- grepl("^q_full\\[", res_tab$name)
not_estimated_q <- is_q & (is.na(res_tab$z) | is.na(res_tab$p_value))

res_tab$display_est <- res_tab$estimate
res_tab$display_se  <- res_tab$se

res_tab$display_est[not_estimated_q] <- 0
res_tab$display_se[not_estimated_q]  <- NA_real_

# ------------------------------------------------------------
# 5) Split years into groups of 5
# ------------------------------------------------------------
year_groups <- split(years_all, ceiling(seq_along(years_all) / 5))

# ------------------------------------------------------------
# 6) Helper: choose width depending on number of year columns
# ------------------------------------------------------------
get_scale_width <- function(n_years) {
  if (n_years >= 5) return("1\\linewidth")
  if (n_years == 4) return("0.86\\linewidth")
  if (n_years == 3) return("0.72\\linewidth")
  if (n_years == 2) return("0.58\\linewidth")
  return("0.46\\linewidth")
}

# ------------------------------------------------------------
# 7) Build one compact LaTeX table
# ------------------------------------------------------------
make_compact_latex_table <- function(year_vec, table_index, gap_pt = 16) {
  
  scale_width <- get_scale_width(length(year_vec))
  
  out_df <- data.frame(
    Quantity = unname(label_map[param_order]),
    stringsAsFactors = FALSE
  )
  
  for (yy in year_vec) {
    tmp <- res_tab[res_tab$year == yy, ]
    tmp <- tmp[match(param_order, tmp$name), ]
    
    est_fmt <- vapply(tmp$display_est, fmt_e, character(1))
    se_fmt  <- vapply(tmp$display_se, function(x) {
      if (is.na(x) || !is.finite(x)) return("-")
      fmt_e(x)
    }, character(1))
    
    out_df[[paste0("est_", yy)]] <- est_fmt
    out_df[[paste0("se_", yy)]]  <- se_fmt
  }
  
  # Wider, clearer gap between year groups
  col_spec <- "l"
  for (j in seq_along(year_vec)) {
    col_spec <- paste0(col_spec, "cc")
    if (j < length(year_vec)) {
      col_spec <- paste0(col_spec, "@{\\hspace{", gap_pt, "pt}}")
    }
  }
  ...
  h1_parts <- c("Quantity")
  for (yy in year_vec) {
    h1_parts <- c(h1_parts, paste0("\\multicolumn{2}{c}{", yy, "}"))
  }
  h1 <- paste(h1_parts, collapse = " & ")
  h1 <- paste0(h1, " \\\\")
  
  # Header line 2
  h2_parts <- c("")
  for (yy in year_vec) {
    h2_parts <- c(h2_parts, "$\\hat{\\theta}$", "SE")
  }
  h2 <- paste(h2_parts, collapse = " & ")
  h2 <- paste0(h2, " \\\\")
  
  # cmidrules
  cmids <- character(length(year_vec))
  for (j in seq_along(year_vec)) {
    left  <- 2 + (j - 1) * 2
    right <- left + 1
    cmids[j] <- paste0("\\cmidrule(lr){", left, "-", right, "}")
  }
  
  # body
  body_lines <- apply(out_df, 1, function(r) paste(r, collapse = " & "))
  body_lines <- paste0(body_lines, " \\\\")
  
  year_txt <- if (length(year_vec) == 1) {
    as.character(year_vec)
  } else {
    paste0(min(year_vec), "--", max(year_vec))
  }
  ...
  lines <- c(
    ...
    "\\centering",
    paste0(
      "\\caption{Estimated quantities and standard errors for years ", year_txt,
      ". Coefficients displayed as 0 with SE shown as - correspond to spline coefficients not estimated in that year. ",
      "The quantity $s_y^{*}$ is not directly estimated; it is obtained by profiling and is reported for clarity.}"
    ),
    paste0("\\label{tab:appendix_params_", table_index, "}"),
    "\\begingroup",
    "\\setlength{\\tabcolsep}{3.6pt}",
    "\\renewcommand{\\arraystretch}{0.92}",
    "\\scriptsize",
    paste0("\\resizebox{", scale_width, "}{!}{%"),
...
for (i in seq_along(year_groups)) {
  yrs <- year_groups[[i]]
  tab_lines <- make_compact_latex_table(
    year_vec = yrs,
    table_index = i,
    gap_pt = 16
  )
  
  out_file <- file.path("tables_dir", paste0("appendix_param_table_", i, ".tex"))
  writeLines(tab_lines, out_file)
  
  master_lines <- c(
    master_lines,
    paste0("% ---- Table ", i, " ----"),
    paste0("\\input{tables_dir/appendix_param_table_", i, ".tex}"),
    ""
  )
}

master_file <- file.path("tables_dir", "appendix_all_param_tables.tex")
writeLines(master_lines, master_file)

cat("Created ", length(year_groups), " appendix tables in 'tables_dir'\n", sep = "")
cat("Master LaTeX file: ", master_file, "\n", sep = "")
################################################################
################################################################
...
# --------------------------------------------
# Extract elbow-method quantities
# --------------------------------------------
df_elbow <- data.frame(
  k = km_out$k_values,
  wss = km_out$wss
...
# Percent reduction in WSS when moving from k-1 to k
df_elbow$percent_drop_from_previous <- c(
  NA,
  100 * (head(df_elbow$wss, -1) - tail(df_elbow$wss, -1)) / head(df_elbow$wss, -1)
)

cat("\n--- Elbow table ---\n")
print(df_elbow)

# Specific quantities often useful in the paper
wss_k1 <- df_elbow$wss[df_elbow$k == 1]
wss_k2 <- df_elbow$wss[df_elbow$k == 2]

drop_1_to_2 <- 100 * (wss_k1 - wss_k2) / wss_k1

cat("\n--- Key elbow quantities ---\n")
cat("WSS at k=1:", round(wss_k1, 3), "\n")
cat("WSS at k=2:", round(wss_k2, 3), "\n")
cat("Percent reduction from k=1 to k=2:", round(drop_1_to_2, 1), "%\n")

# Optional: compute all successive drops as a named vector
successive_drops <- setNames(
  round(df_elbow$percent_drop_from_previous[-1], 1),
  paste0(df_elbow$k[-1] - 1, "->", df_elbow$k[-1])
)

cat("\n--- Successive % drops in WSS ---\n")
print(successive_drops)

# ============================================
# Quantitative comparison between the 2 clusters
# ============================================

# Rebuild biomass table
b_idx <- years - 1971 - 1
...
df_bio <- data.frame(
  ...
  age3_biomass = as.numeric(data_B[3, b_idx]),
  total_biomass = as.numeric(data_B[5, b_idx]),
  cluster = factor(km_out$idx)
)

# 1) Number of years in each cluster
cat("\n--- Number of years per cluster ---\n")
print(table(df_bio$cluster))

# 2) Mean, median, sd, min, max for biomass by cluster
cluster_summary <- aggregate(
  cbind(age3_biomass, total_biomass) ~ cluster,
  data = df_bio,
  FUN = function(x) c(
...
cat("\n--- Biomass summary by cluster ---\n")
print(cluster_summary)

# 3) Simpler readable summaries
cat("\n--- Mean biomass by cluster ---\n")
print(aggregate(cbind(age3_biomass, total_biomass) ~ cluster, df_bio, mean))

cat("\n--- Median biomass by cluster ---\n")
print(aggregate(cbind(age3_biomass, total_biomass) ~ cluster, df_bio, median))

cat("\n--- SD biomass by cluster ---\n")
print(aggregate(cbind(age3_biomass, total_biomass) ~ cluster, df_bio, sd))

# 4) Ratios of cluster 2 to cluster 1
mean_vals <- aggregate(cbind(age3_biomass, total_biomass) ~ cluster, df_bio, mean)

age3_ratio  <- mean_vals$age3_biomass[mean_vals$cluster == 2] /
  mean_vals$age3_biomass[mean_vals$cluster == 1]

total_ratio <- mean_vals$total_biomass[mean_vals$cluster == 2] /
  mean_vals$total_biomass[mean_vals$cluster == 1]

cat("\n--- Ratio of mean biomass: cluster 2 / cluster 1 ---\n")
cat("Age-3 biomass ratio =", round(age3_ratio, 3), "\n")
cat("Total biomass ratio =", round(total_ratio, 3), "\n")

# 5) t-tests (simple quantitative comparison)
cat("\n--- Two-sample t-tests ---\n")
print(t.test(age3_biomass ~ cluster, data = df_bio))
print(t.test(total_biomass ~ cluster, data = df_bio))

# 6) Nonparametric Wilcoxon tests (safer if distributions are skewed)
cat("\n--- Wilcoxon rank-sum tests ---\n")
print(wilcox.test(age3_biomass ~ cluster, data = df_bio, exact = FALSE))
print(wilcox.test(total_biomass ~ cluster, data = df_bio, exact = FALSE))

# 7) Long-term median split:
#    how often each cluster falls above/below overall median biomass
med_age3  <- median(df_bio$age3_biomass, na.rm = TRUE)
med_total <- median(df_bio$total_biomass, na.rm = TRUE)

df_bio$age3_above_median  <- ifelse(df_bio$age3_biomass  > med_age3,  "above", "below_or_equal")
df_bio$total_above_median <- ifelse(df_bio$total_biomass > med_total, "above", "below_or_equal")

cat("\n--- Cluster vs age-3 biomass median split ---\n")
print(table(df_bio$cluster, df_bio$age3_above_median))

cat("\n--- Cluster vs total biomass median split ---\n")
print(table(df_bio$cluster, df_bio$total_above_median))

# Row proportions (easy to describe in text)
cat("\n--- Row proportions: cluster vs age-3 biomass median split ---\n")
print(round(prop.table(table(df_bio$cluster, df_bio$age3_above_median), 1), 3))

cat("\n--- Row proportions: cluster vs total biomass median split ---\n")
print(round(prop.table(table(df_bio$cluster, df_bio$total_above_median), 1), 3))

# 8) Add avg_index values explicitly
avg_index_df <- data.frame(
  cluster = c(1, 2),
  avg_peak_length_class = c(25.0000, 21.5625)
)

cat("\n--- Average peak length-class index by cluster ---\n")
print(avg_index_df)
...
##############################################
#reviewer comment about compairing mean lengths
###############################################

# ============================================================
# Simple mean-length-at-age summary by year
# ============================================================
...
df_meanlen <- data.frame(
  year = years,
  mean_age2_y = NA_real_,
  mean_age3_y1 = NA_real_,
  delta_mean_length = NA_real_,
  cluster = km_out$idx
)
...
for (ii in seq_along(years)) {
  ...
  N_age2_y   <- get_age_year(data_N32, 2, y)       # age 2 in year y
  N_age3_y1  <- get_age_year(data_N32, 3, y + 1)   # age 3 in year y+1
  
  m2 <- weighted_mean_safe(x_mid, N_age2_y)
  m3 <- weighted_mean_safe(x_mid, N_age3_y1)
  
  df_meanlen$mean_age2_y[ii] <- m2
  df_meanlen$mean_age3_y1[ii] <- m3
  df_meanlen$delta_mean_length[ii] <- m3 - m2
}

cat("\n--- Mean-length summary by year ---\n")
print(df_meanlen)

# ============================================================
# Summary of delta mean length by cluster
# ============================================================

cat("\n--- Mean delta by cluster ---\n")
print(aggregate(delta_mean_length ~ cluster, data = df_meanlen, mean))

cat("\n--- Median delta by cluster ---\n")
print(aggregate(delta_mean_length ~ cluster, data = df_meanlen, median))

cat("\n--- SD of delta by cluster ---\n")
print(aggregate(delta_mean_length ~ cluster, data = df_meanlen, sd))

cat("\n--- t-test for delta mean length by cluster ---\n")
print(t.test(delta_mean_length ~ factor(cluster), data = df_meanlen))

cat("\n--- Wilcoxon test for delta mean length by cluster ---\n")
print(wilcox.test(delta_mean_length ~ factor(cluster), data = df_meanlen, exact = FALSE))


# ============================================================
# Boxplot of delta mean length by cluster
# ============================================================

p_box <- ggplot(df_meanlen, aes(x = factor(cluster), y = delta_mean_length)) +
  geom_boxplot() +
  labs(
    x = "Cluster",
    y = expression(bar(l)[3,y+1] - bar(l)[2,y])
  ) +
...
ggsave(
  filename = file.path("results_plots", "delta_mean_length_boxplot.png"),
